package com.action.evn.evnErrorBuffer;
...
public class AEvnErrorBuffer extends ACore {
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		FEvnErrorBuffer bean = (FEvnErrorBuffer) form;
		...
		if (!errors.isEmpty()) {
			...
			BEvnErrorBuffer apps = new BEvnErrorBuffer();
			if (apps.update(bean)) {
		...
		request.setAttribute("BOptionModule", new BEvnModuleBuffer().getListRecord());
		request.setAttribute("BTables", bean);
		request.setAttribute("BEvnErrorBuffer", bean);
	...
	private String validate(FEvnErrorBuffer bean, String anchor,
			ActionErrors errors) {
		if (anchor.equals(_VIEW)) {
			...
			if ((bean.getError_Code().trim().equals(_BLANK) || bean
					.getError_Description().trim().equals(_BLANK))) {
				errors.add("alert", new ActionError("errors.apps.edit.short"));
			}
		} else if (anchor.equals(_DELETE)) {
			if (bean.getError_Id() <= 0) {
