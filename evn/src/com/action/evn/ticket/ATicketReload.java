...
public class ATicketReload extends ACore {
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		if (!errors.isEmpty()) {
			...
			request.setAttribute("BTables", bean);
			bean.reset();
			bean.setStart_time(DateProc.getYYYYMMDDHHMMString(DateProc
					.createTimestamp()));
			bean.setEnd_time(DateProc.getYYYYMMDDHHMMString(DateProc
					.createTimestamp()));
			target = "_PREPARED";
			...
			beanImport1
					.setNameSQL("select TICKET_ID,decode(trunc(evn_time-sysdate),0,to_char(evn_time,'HH24:mi:ss'),trunc(evn_time-sysdate)) as EVN_TIME,"
							+ "decode(trunc(start_time-sysdate),0,to_char(start_time,'HH24:mi:ss'),trunc(start_time-sysdate)) as START_TIME,ROUND((end_time-start_time)*24)||' hours' as END_TIME,TOTAL_RECORDS,CODE,TABLE_NAME,EVN_TIME as t1,START_TIME as t2,END_TIME as t3 from "
							+ ITables.TABLE_TICKET_BUFFER
							+ " where STATUS='O' and ACTIVE= -1 and START_TIME<= sysdate and END_TIME+ numtodsinterval("
							+ IKey.getDelay()
							+ ", 'MINUTE')>=sysdate order by START_TIME desc");
			// beanImport1.setNameSQL("select TICKET_ID,EVN_TIME,START_TIME,END_TIME,TOTAL_RECORDS,CODE,TABLE_NAME from "
			// +ITables.TABLE_TICKET_BUFFER +
			...
			beanImport1
					.setNameSQL("select TICKET_ID,decode(trunc(evn_time-sysdate),0,to_char(evn_time,'HH24:mi:ss'),trunc(evn_time-sysdate)) as EVN_TIME,"
							+ "decode(trunc(start_time-sysdate),0,to_char(start_time,'HH24:mi:ss'),trunc(start_time-sysdate)) as START_TIME,ROUND((end_time-start_time)*24)||' hours' as END_TIME,TOTAL_RECORDS,CODE,TABLE_NAME,EVN_TIME as t1,START_TIME as t2,END_TIME as t3 from "
							+ ITables.TABLE_TICKET_BUFFER
							+ " where STATUS='O' and active=0 and START_TIME<= sysdate and END_TIME+ numtodsinterval("
							+ IKey.getDelay()
			...
			request.setAttribute("BPaging2",
					new BReportBuffer().getPaging(beanImport1));
			String sql = "";
			String sqlN = " where STATUS='" + bean.getSearch_type()
					+ "' and to_char(" + IFields.TICKET_START_TIME
					+ ",'dd/mm/yyyy')='" + bean.getSearch_time() + "'";
			String sqlT = " where STATUS='"
					+ bean.getSearch_type()
					+ "' and to_char("
					+ IFields.TICKET_START_TIME
					+ ",'mm/yyyy')='"
					+ bean.getSearch_time().substring(
							bean.getSearch_time().indexOf("/") + 1) + "'";
			String sqlNa = " where STATUS='"
					+ bean.getSearch_type()
					+ "' and to_char("
					+ IFields.TICKET_START_TIME
					+ ",'yyyy')='"
					+ bean.getSearch_time().substring(
							bean.getSearch_time().length() - 4) + "'";
			if (bean.getSearch_type_time() == 0) {
				sql = " where STATUS='" + bean.getSearch_type()
						+ "' and to_char(" + IFields.TICKET_START_TIME
						+ ",'dd/mm/yyyy')='" + bean.getSearch_time() + "'";
			...
			beanImport1.setNameSQL("select * from "
					+ ITables.TABLE_TICKET_BUFFER + sql);
			request.setAttribute("BColumnsList",
					new BReportBuffer().getAllColumnsInTable(beanImport1));
			request.setAttribute("BRecordList",
					new BReportBuffer().getAllRecord(beanImport1));
			request.setAttribute("BPaging",
					new BReportBuffer().getPaging(beanImport1));
			...
			beanImport1.setNameSQL("select * from "
					+ ITables.TABLE_TICKET_BUFFER + sqlN);
			request.setAttribute("BTotalDate",
					new BReportBuffer().getPaging(beanImport1));
			beanImport1.setNameSQL("select * from "
					+ ITables.TABLE_TICKET_BUFFER + sqlT);
			request.setAttribute("BTotalMonth",
					new BReportBuffer().getPaging(beanImport1));
			beanImport1.setNameSQL("select * from "
					+ ITables.TABLE_TICKET_BUFFER + sqlNa);
		...
		if (anchor.equals("_VIEW_TICKET")) {
			...
			request.setAttribute("BPaging2",
					new BReportBuffer().getPaging(beanImport));
			beanImport.setNameSQL("select TICKET_ID from "
		...
		request.setAttribute("BSourceAccountBuffer",
				new BSourceAccountBuffer().getAllRecord());
